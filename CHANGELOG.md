...
- OpenAPI operation layout and schema tree. Each endpoint gets a method badge, its path with parameters marked, parameters grouped by where they go, a request body tabbed by content type, security explained in place, and responses tabbed by status. Wide screens get two columns with the response panel sticky inside its own operation, so it lets go at the next endpoint rather than riding over it. Schemas render as an expandable tree with types read for 3.0 and 3.1 alike: `array of Pet`, `string | null`, formats, enums, `oneOf` as named alternatives, and a marker where a schema contains itself. All of it is computed at build time; nothing about a schema runs in the browser
