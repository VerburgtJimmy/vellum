...
- `:::preview[view-name]` renders one of your Blade views in the page, beside its own highlighted source. Views resolve only from `previews.path`, so a docs page cannot name an arbitrary file. Each preview renders in its own frame carrying `previews.stylesheets`, which is how it can load your application's compiled CSS without restyling the documentation around it, and how the docs theme is kept out of the component. The frame follows light and dark mode and reports its own height. `height`, `padding` and `code="false"` adjust it. A preview compiles once at build time, so a view that queries the database fails the build rather than freezing one row of data into the page, and a missing view or a view that throws fails naming both the view and the docs page. Static export carries previews with no extra files, since the frame document travels inside the page
- Two example preview views ship with the package and are published by `vellum:install` alongside a stub page that uses them. A name Vellum cannot find in `previews.path` falls back to those, which is what lets the documentation show working previews in any installation rather than describing them; a view of your own with the same name takes precedence
