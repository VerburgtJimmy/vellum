...
- Search results are sections: a heading and the text under it, shown with the page title and heading, and linking to that heading. At most two sections of one page are listed before other pages' sections
...
- The same two files at `/llms.txt` and `/llms-full.txt`, unless the app already routes those paths. They follow `route.domain`, and `agents.llms_txt_root` turns them off
- `vellum:export` writes `llms.txt` and `llms-full.txt` at the export root
- Content negotiation. A docs page requested with an `Accept` header that prefers `text/markdown` returns the page's raw Markdown. Browsers, whose default `Accept` does not name `text/markdown`, still get HTML, and page responses send `Vary: Accept`. `agents.content_negotiation` turns it off
- Every page links to its raw Markdown with `<link rel="alternate" type="text/markdown">` and a matching `Link` response header
- Raw Markdown responses send `Link: <page>; rel="canonical"` for the HTML page, `Last-Modified` when the page has a date, and `X-Vellum-Docs-Version` when versions are enabled
- A last-updated date for each page, from an `updated` frontmatter key or the file's last git commit. It appears under the page title, as `<lastmod>` in the sitemap, as `Last-Modified` on raw Markdown and in `llms-full.txt`. Pages without either have no date. Shallow clones are treated as having no git history
- The changelog has a raw Markdown URL at `{prefix}/_vellum/raw/changelog.md`, supports content negotiation, and is included in `llms.txt`, `llms-full.txt` and the sitemap. With `changelog.unreleased` off, the `[Unreleased]` section is left out of the raw Markdown too
- `checks.search`, `checks.search_min`, `agents.llms_txt`, `agents.llms_txt_root`, `agents.content_negotiation` and `agents.search` in `config/vellum.php`
