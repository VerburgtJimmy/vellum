...
- A questions file, `questions.yml` next to your Markdown, turns the questions readers actually ask into a build check. Each entry names the question, the page and the heading that answers it, with an `also` list for other places that answer it equally well. `vellum:build` asks them against the index it just wrote and reports how many find their answer in the top five. `checks.search_min`, or `VELLUM_SEARCH_MIN`, sets the share that must, and a build below it fails. A question whose target no longer exists is reported as a broken question rather than a miss, left out of the count, and fails a strict build. The check reads the copy a guest gets, so it does not depend on who runs the build. `checks.search` turns it off and `vellum:build --check-search` turns it back on for one run
- `checks.search` and `checks.search_min` in `config/vellum.php`, and `--check-search` on `vellum:build`
- `{prefix}/_vellum/answer?q=`, one question answered as JSON: the answer card when search is sure of it, by the same `answers.card_threshold` the dialog uses, and the top five sections with their URL, heading path, passage and last-updated date. It runs the same ranker the browser runs, on the copy the caller may see, so an agent and a reader get the same answer and the same access rules. An empty question is a 400, and `agents.answer` turns the route off. A static export has no endpoint, since it has no server

### Changed
- `answers.card_threshold` defaults to 0.75 rather than 0.65. A card states an answer, so a wrong one costs more than a missing one: at 0.75 every card on the golden set names the right section, against nine in ten before, at the price of about half the cards
- The theming page's "The rest" section is now four sections, one per setting, and the writing guide asks for headings that are answer-shaped: one thing a reader would ask about per heading. Search indexes and answers one section at a time, so a heading covering four unrelated settings can only half-answer each of them
