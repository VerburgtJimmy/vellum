...
```bash
php artisan vellum:build [--docs-version=v1] [--strict]
```
...
Writes a complete static site: HTML for every page, the assets, the raw Markdown, a
MiniSearch index, a changelog feed if a changelog is configured, and a `404.html` at the
root.
