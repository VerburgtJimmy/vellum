...
The command writes the HTML pages, assets, a MiniSearch index and a `sitemap.xml` to `public/docs-static`. Change the directory with `export.out`, or pass `--out=` for a single run.
...
The export always uses MiniSearch, even when `search.driver` is `scout`. Gated pages are left out, and the command logs each one it drops.
