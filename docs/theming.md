...
`default` sets the initial mode: `light`, `dark` or `system`. Readers can change it from
the theme menu, and their choice is stored in `localStorage`.

`radius` sets the `--radius` CSS variable on the document.

The optional `'fonts'` key injects HTML into the layout head, for example a `<link>` tag
for a web font.
